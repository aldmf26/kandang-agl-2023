<div class="row">
    ...
    <div class="col-lg-12">
        <table class="table table-bordered text-center">
            ...
            <tr>
                <th class="dhead" width="9%">Produk</th>
                <th class="dhead" width="9%">Pcs</th>
                <th class="dhead" width="9%">Kg Kotor</th>
            ...
            <tr>
                ...
                <td>
                    <input value="{{ $cek->ikat ?? 0 }}" type="text" name="pcs[]" class="form-control text-end"
                        count="{{ $i + 1 }}">
                </td>
                <td>
                    <input value="{{ $cek->ikat_kg ?? 0 }}" type="text" name="kg[]" class="form-control text-end">
                </td>
                {{-- <td>
                ...
                {{-- <td>
                    <input type="text" value="{{ !empty($cek) ? ($cek->pcs == 0 ? 0 : $cek->potongan_pcs ?? 0) : 0 }}"
                        readonly class="form-control potongan{{ $i + 1 }}" name="potongan_pcs[]">
                </td>
                <td>
                    <input type="text" value="{{ !empty($cek) ? ($cek->pcs == 0 ? 0 : $cek->ttl_kg_pcs ?? 0) : 0 }}"
                        readonly class="form-control ttlKgPcs{{ $i + 1 }}" name="ttl_kg_pcs[]">
                </td> --}}
            </tr>
            @endforeach
        </table>
